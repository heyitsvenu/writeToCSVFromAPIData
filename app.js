  ...
  axios
    ...
    .then((res) => {
      ...
      const mappedData = listings.map((item) => {
        ...
        const date1 = new Date(item.rateSummary.beginDate).getTime();
        const date2 = new Date().getTime();
        const index = Math.floor((date2 - date1) / (1000 * 3600 * 24));
        let newRentNights = rateSummary.rentNights.slice(index);
        let dateToAdd = add(new Date(), {
          years: 0,
          months: 0,
          weeks: 0,
          days: -1,
        ...
        newRentNights = newRentNights.map((rentPerNight) => {
          dateToAdd = add(dateToAdd, {
            years: 0,
            months: 0,
            weeks: 0,
            days: 1,
          ...
          return [format(dateToAdd, 'MM/dd/yy'), rentPerNight];
        });
        return [['Unit Id', listingId], ['Name', headline], ...newRentNights];
      });
      fastcsv.write(mappedData, { headers: true }).pipe(ws);
    })
    .catch((error) => {
      console.error(error);
    });
